...
- `CloneError` with `code: 'UNSUPPORTED_TYPE'` and `Error.cause` support, thrown for functions, `Promise`, `Intl.*`, `WeakMap`, `WeakSet`, and constructor functions.
- Browser-safe — `Buffer` is captured behind a runtime guard so the package works in non-Node bundles.
...
- CI wired through `coroboros/ci/.github/workflows/javascript-npm-packages.yml@v0` with OIDC Trusted Publisher and `npm provenance`.
